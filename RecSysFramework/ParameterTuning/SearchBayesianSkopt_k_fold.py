...
class Recommender_k_Fold_Wrapper():
     ...
     def __init__(self, recommender_class, n_folds, recommender_input_args_list, verbose=True):
         ...
         assert n_folds is not None and n_folds >= 1, \
            "{}: number of folds must be an integer value greater than 1, provided value was '{}'"\
            .format(self.RECOMMENDER_NAME, str(n_folds))

         assert n_folds == len(recommender_input_args_list), \
            "{}: inconsistent values for number of folds '{}' and the length of constructor_data_list '{}'"\
            .format(self.RECOMMENDER_NAME, n_folds, len(recommender_input_args_list))
...
class Evaluator_k_Fold_Wrapper():
    ...
    def __init__(self, evaluator_instance_list, n_folds, verbose=True):
        ...
        assert n_folds is not None and n_folds >= 1, \
            "{}: number of folds must be an integer value greater than 1, provided value was '{}'"\
            .format(self.EVALUATOR_NAME, str(n_folds))

        assert n_folds == len(evaluator_instance_list), \
            "{}: inconsistent values for number of folds '{}' and the length of evaluator_instance_list '{}'"\
            .format(self.EVALUATOR_NAME, n_folds, len(evaluator_instance_list))
...
#         assert n_folds is not None and n_folds >= 1, \
#             "{}: number of folds must be an integer value greater than 1, provided value was '{}'"\
#             .format(self.ALGORITHM_NAME, str(n_folds))
#
#         self.n_folds = n_folds
#
#         self.recommender_class = recommender_class
...
#
#         super(SearchBayesianSkopt_k_fold, self)._init_metadata_dict(n_cases)
#
#         for fold_index in range(self.n_folds):
