...
class SearchInputRecommenderArgs(object):
    ...
    def add_contructor_positional_args(self, newargs):
        if not isinstance(newargs, list):
            newargs = [newargs]
        self.CONSTRUCTOR_POSITIONAL_ARGS += newargs


    def add_contructor_keyword_args(self, newargs):
        assert isinstance(newargs, dict), \
            "Dictionary required. Can not automatically convert args of type {}".format(type(newargs))
        self.CONSTRUCTOR_KEYWORD_ARGS = dict(**self.CONSTRUCTOR_KEYWORD_ARGS, **newargs)


    def add_fit_positional_args(self, newargs):
        if not isinstance(newargs, list):
            newargs = [newargs]
        self.FIT_POSITIONAL_ARGS += newargs


    def add_fit_keyword_args(self, newargs):
        assert isinstance(newargs, dict), \
            "Dictionary required. Can not automatically convert args of type {}".format(type(newargs))
        self.FIT_KEYWORD_ARGS = dict(**self.FIT_KEYWORD_ARGS, **newargs)
...
def get_result_string_evaluate_on_validation(results_run_single_cutoff, n_decimals=7):
    ...
    for metric in results_run_single_cutoff.keys():
        output_str += "{}: {:.{n_decimals}f}, "\
            .format(metric, results_run_single_cutoff[metric], n_decimals=n_decimals)
...
class SearchAbstractClass(object):
    ...
    def _extend_metadata_dict(self, n_calls):
        for k in self.metadata_dict.keys():
            if isinstance(self.metadata_dict[k], list):
                if n_calls > len(self.metadata_dict[k]):
                    self.metadata_dict[k].extend([None] * (n_calls - len(self.metadata_dict[k])))
    ...
    def _evaluate_on_validation(self, current_fit_parameters):
        ...
        if isinstance(recommender_instance, EarlyStoppingModel):
            ...
            result_dict = fed["best_results_run"]
            evaluation_time = fed["best_results_evaluation_time"]
        else:
            start_time = time.time()
            # Evaluate recommender and get results for the first cutoff
            metric_handler = self.evaluator_validation.evaluateRecommender(recommender_instance)
            result_dict = metric_handler.get_results_dictionary(use_metric_name=True)
            evaluation_time = time.time() - start_time

        result_string = get_result_string_evaluate_on_validation(result_dict[self.cutoff_to_optimize], n_decimals=7)
    ...
    def _evaluate_on_test_with_data_last(self):
        ...
        self._print("{}: Evaluation with constructor data for final test. Using best config: {}"
                    .format(self.ALGORITHM_NAME, self.metadata_dict["hyperparameters_best"]))

        # Use the hyperparameters that have been saved
        assert self.metadata_dict["hyperparameters_best"] is not None, \
    ...
    def _objective_function(self, current_fit_parameters_dict):
        ...
        try:
            ...
            current_result = self._calculate_metric_to_optimize(result_dict[self.cutoff_to_optimize])
            ...
            if isinstance(recommender_instance, EarlyStoppingModel):
                ...
                epoch_value = n_epochs_early_stopping_dict["epochs"]
                current_fit_parameters_dict["epochs"] = epoch_value

                # rewrite updated epochs parameter
                self.metadata_dict["hyperparameters_list"][self.model_counter] = current_fit_parameters_dict.copy()
            ...
            if self.metadata_dict["result_on_validation_best"] is None:
                ...
                best_solution_val = self._calculate_metric_to_optimize(self.metadata_dict["result_on_validation_best"][self.cutoff_to_optimize])
                new_best_config_found = current_result < best_solution_val
            ...
            cs_op = getattr(recommender_instance, "clear_session", None)
    ...
    def _calculate_metric_to_optimize(self, result_dictionary):
        current_result = 0.
        if isinstance(self.metric_to_optimize, list):
            for metric in self.metric_to_optimize:
                current_result += result_dictionary[metric] / len(self.metric_to_optimize)
        elif isinstance(self.metric_to_optimize, dict):
            for metric, value in self.metric_to_optimize.items():
                current_result += result_dictionary[metric] * value
        else:
            current_result = result_dictionary[self.metric_to_optimize]
