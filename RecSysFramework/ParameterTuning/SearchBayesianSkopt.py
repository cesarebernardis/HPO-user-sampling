...
class SearchBayesianSkopt(SearchAbstractClass):
    ...
    def _resume_from_saved(self):
        ...
        try:
            ...
            self._extend_metadata_dict(self.n_calls)
        ...
        while self.model_counter < len(hyperparameters_list_saved) and \
            ...
            for index in range(len(self.hyperparams_names)):
                ...
                if isinstance(self.hyperparams_values[index], Categorical) and \
                        self.hyperparams_values[index].transformed_size == 1:
            ...
            if validation_result is None:
                ...
                result_on_validation_list_input.append(self._calculate_metric_to_optimize(result_on_validation_list_saved[self.model_counter][self.cutoff_to_optimize]))

            self.model_counter += 1

        self._print("{}: Resuming '{}'... Loaded {} configurations."
                    .format(self.ALGORITHM_NAME, self.output_file_name_root, self.model_counter))
    ...
    def search(self, recommender_input_args,
        ...
        :param metric_to_optimize:  string  name of the metric to optimize
                                    list    list of metrics to optimize, equally weighted
                                    dict    metrics to optimize as keys, weights of the metrics as values
        :param n_cases:
        ...
        for name, hyperparam in self.parameter_search_space.items():
            ...
            if any(isinstance(hyperparam, sko_type) for sko_type in skopt_types):
                ...
                raise ValueError("{}: Unexpected parameter type: {} - {}"
                                 .format(self.ALGORITHM_NAME, str(name), str(hyperparam)))
